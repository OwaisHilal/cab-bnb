...
2. `notify_vendor_booking` (`template_key` `vendor_assign_driver_v1`, MSG91 name `vendor_assign_driver_v3`, **UTILITY**, approved 2026-09-24) — selected vendor POC only. Env still `MSG91_VENDOR_NOTIFY_TEMPLATE_*` (default name now `vendor_assign_driver_v3`, see `lib/whatsapp/templateEnv.ts`). Do not mutate approved `vendor_booking_notify_v1`. v3 keeps the same 9 body variables and the real Meta-approved "Assign driver" URL button from v2 — `button_1` on the bulk send carries the signed token as the dynamic suffix (`buildVendorAssignTokenAndUrl` in `lib/whatsapp/notifyVendorBooking.ts` / `supabase/functions/_shared/vendorAssignToken.ts`; template URL is fixed as `.../vendor/assign-driver?token={{1}}`) — only the trailing CTA sentence is simplified (see `docs/2026-09-24-vendor-assign-driver-v3-template.md`). Session `cta_url` ("Assign driver" button) remains the fallback if the bulk template send fails or the flag is off — same pattern as `create_ride_group`'s "Join ride group" button. The CTA opens `app/vendor/assign-driver?token=…` (`lib/whatsapp/vendorAssignToken.ts`, signed + expiring, no vendor login exists), a mobile form for driver/vehicle details that posts to `POST /api/vendor/assign-driver` and shares `lib/whatsapp/assignDriverToBooking.ts` with the legacy path. Vendor can still reply with a whole-body 10-digit mobile, or `DRIVER: name | phone | vehicle | model`, as a fallback — `parse_driver_details` uses the same shared helper. See `docs/2026-09-21-vendor-assign-driver-v2-template.md` for the creation + approval + wiring history.
3. `send_balance_payment` — session `payment_link` (`driver_assigned_payment_v1`) for `tripTotal − ₹99`. Header image is a public HTTPS Storage URL from bucket `driver-cards` (compose via Node `POST /api/internal/driver-card`, `APP_URL` + `CRON_SECRET`). **Never** put Pay Now on a Utility template. If the image send fails, retry `payment_link` without image. `localhost` / `/public` paths will not fetch in production; do not use short-lived signed URLs.
   ...
   **Driver/car media card** (`lib/drivers/composeDriverCard.ts`) — composes `drivers.photo_url` + `vehicles.stock_photo_url` into one JPEG with `sharp`, uploads it to the public `driver-cards` bucket, and sends it as a plain image message (`sendWhatsAppImageMessage`) immediately before the `Pay Balance` CTA in `lib/whatsapp/sendBalancePaymentLink.ts`. Source priority for each photo: (1) `drivers.photo_url` / `vehicles.stock_photo_url` as a public HTTPS URL, (2) that same column as a `/public`-relative path (local/demo only — see (1)'s caveat above), (3) local demo fallback by driver name (`public/demo/drivers/`) or vehicle type code (`public/fleet/`) — see the `README.md` in each folder for expected filenames, (4) a neutral placeholder card with no driver portrait. Every remote/local buffer is verified to actually decode as an image before compositing; a bad URL or missing asset degrades gracefully to the next fallback instead of failing the send. This image is best-effort and never blocks the payment CTA — its attempt/success/error is still recorded on the `whatsapp_message_log` row's `button_payload.media` field for admin/debug replay.
