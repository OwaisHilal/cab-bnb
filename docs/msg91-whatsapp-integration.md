...
2. `notify_vendor_booking` (`template_key` `vendor_assign_driver_v1`, MSG91 name `vendor_assign_driver_v2`, **UTILITY**, approved 2026-09-23) — selected vendor POC only. Env still `MSG91_VENDOR_NOTIFY_TEMPLATE_*` (default name now `vendor_assign_driver_v2`, see `lib/whatsapp/templateEnv.ts`). Do not mutate approved `vendor_booking_notify_v1`. v2 has a real Meta-approved "Assign driver" URL button — `button_1` on the bulk send carries the signed token as the dynamic suffix (`buildVendorAssignTokenAndUrl` in `lib/whatsapp/notifyVendorBooking.ts` / `supabase/functions/_shared/vendorAssignToken.ts`; template URL is fixed as `.../vendor/assign-driver?token={{1}}`). Session `cta_url` ("Assign driver" button) remains the fallback if the bulk template send fails or the flag is off — same pattern as `create_ride_group`'s "Join ride group" button. The CTA opens `app/vendor/assign-driver?token=…` (`lib/whatsapp/vendorAssignToken.ts`, signed + expiring, no vendor login exists), a mobile form for driver/vehicle details that posts to `POST /api/vendor/assign-driver` and shares `lib/whatsapp/assignDriverToBooking.ts` with the legacy path. Vendor can still reply with a whole-body 10-digit mobile, or `DRIVER: name | phone | vehicle | model`, as a fallback — `parse_driver_details` uses the same shared helper. See `docs/2026-09-21-vendor-assign-driver-v2-template.md` for the creation + approval + wiring history.
