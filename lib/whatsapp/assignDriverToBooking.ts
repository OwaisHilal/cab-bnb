...
/**
 * Driver/vehicle upsert + booking-attach core, extracted from
 * lib/whatsapp/parseDriverDetails.ts so both the free-text WhatsApp
 * `DRIVER: ...` reply (parseDriverDetails.ts) and the "Assign driver" web
 * form (app/api/vendor/assign-driver/route.ts) drive the exact same
 * booking/job outcome instead of two parallel implementations drifting
...
export const findVendorIdByWhatsApp = async (
  supabase: SupabaseClient,
  fromPhone: string,
): Promise<string | null> => {
...
export const findActiveBookingForVendorPhone = async (
  supabase: SupabaseClient,
  fromPhone: string,
): Promise<ResolvedVendorBooking | null> => {
...
/**
 * Loads a specific booking by id for the web-form path (which already
 * knows its bookingId from the signed token) instead of resolving "the
 * vendor's most recent active booking" the way the free-text path does.
 */
export const loadVendorBookingById = async (
  supabase: SupabaseClient,
  bookingId: string,
): Promise<ResolvedVendorBooking | null> => {
  const { data, error } = await supabase
    .from("bookings")
    .select("id, vendor_id, status, lock_type, payment_status, vehicle_type_id")
    .eq("id", bookingId)
    .maybeSingle()
...
/** True once a booking has moved past the point where a driver can still be (re)assigned. */
export const isBookingAssignable = (booking: ResolvedVendorBooking): boolean => {
  return !(
    (ATTACHED_OR_LATER_BOOKING_STATUSES as readonly string[]).includes(booking.status) ||
