import { isMsg91ErrorBody, resolveMsg91SendCredentials } from "./msg91WhatsApp.ts";
import {
...
function groupsBaseUrl(): string {
  return Deno.env.get("MSG91_WHATSAPP_GROUPS_URL")?.trim() || MSG91_WHATSAPP_GROUPS_URL;
}
...
export async function createMsg91WhatsAppGroup(input: {
  ...
  // See withGroupsCreateSlash in msg91GroupApi.ts: the bare collection URL
  // (no trailing slash) 308-redirects to an unreachable internal address.
