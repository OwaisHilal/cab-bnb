...
interface QuoteSnapshotRow {
  ...
  trip_requests: TripRequestEmbed | TripRequestEmbed[] | null;
}
...
/**
 * After Select {vendor} (`BOOK_TOKEN::`), send MSG91 WhatsApp Payments
 * `payment_link` for a fixed ₹99 Cashfree cart. Session-only (24h window).
 */
export async function handleSendTokenPaymentLink(
  ...
  const headerImageUrl = Deno.env.get("MSG91_PAYMENT_LINK_HEADER_IMAGE_URL")?.trim();
  ...
  await logOutboundWhatsAppMessage(supabase, {
    tripRequestId: trip.id,
    quoteSnapshotId,
    touristId: trip.tourist_id ?? undefined,
    vendorId: row.vendor_id,
    ...
    waStatus: "sent",
    templateName: TOKEN_LOCK_PAYMENT_TEMPLATE_KEY,
  });
}
...
async function upsertPaymentIntent(
  ...
  if (existing?.id) {
    return { crqid: existing.id as string, intentReady: true };
  }
