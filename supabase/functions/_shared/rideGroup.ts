import { whatsAppInviteCode } from "./msg91GroupApi.ts";
...
export interface RideGroupInviteSpec {
  templateKey: string;
  bodyText: string;
  footerText: string;
  ctaTitle: string;
...
export function buildRideGroupSubject(input: {
  ...
  return `Ride ${input.bookingRef} · ${guest} · ${vendor}`.slice(0, SUBJECT_MAX);
}
...
export function buildGuestRideGroupInvite(input: {
  ...
  return {
    ...
    ctaTitle: RIDE_GROUP_JOIN_BUTTON_TITLE.slice(0, BUTTON_TITLE_MAX),
