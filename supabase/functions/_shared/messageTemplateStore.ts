/**
 * Edge copy of lib/whatsapp/messageTemplateStore.ts — Deno cannot import Next server-only modules.
 */
...
const FALLBACK_TEMPLATES: WhatsAppMessageTemplateRow[] = [
  ...
  },
  {
    ...
    notes:
      "First WhatsApp contact after phone submit. Buttons are Select {vendorName} (max 20 chars). Body lists trip + top 3 quotes.",
    active: true,
  },
  {
    ...
    wired_in_code: "supabase/functions/_shared/handlers/sendTokenPaymentLink.ts",
    notes:
      "Not a dashboard Utility template. MSG91 session interactive type=payment_link (Cashfree). Cart is a single ₹99 item.",
    active: true,
  },
  {
  ...
  },
  {
    ...
    requires_dashboard_create: true,
    wired_in_code: "supabase/functions/_shared/handlers/createRideGroup.ts",
    notes: "Create Green on MSG91 with CTA URL https://chat.whatsapp.com/{{1}}. Session CTA/text is the fallback.",
    active: true,
  },
  {
    ...
    requires_dashboard_create: true,
    wired_in_code: "supabase/functions/_shared/handlers/createRideGroup.ts",
    notes: "Driver often has no 24h session, so the Utility template is required in production.",
    active: true,
  },
  {
  ...
  {
    template_key: "customer_confirmation_v1",
    msg91_template_name: "customer_confirmation_v1",
    category: "UTILITY",
    send_method: "session_image",
    language_code: "en_US",
    body_template:
