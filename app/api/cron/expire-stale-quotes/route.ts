...
/**
 * Checklist 3.9: worker endpoint invoked hourly by pg_cron, with a
 * Hobby-safe daily Vercel Cron safety net. Secured via CRON_SECRET.
 * Invokes `expire-stale-quotes`, which expires `quote_snapshots` with no
 * response after `QUOTE_EXPIRY_HOURS` — this does not go through
 * `job_queue`.
 */
